...
const Add = ({token}) => {
  ...
  const onSubmit=async(e)=>{
    ...
    try {
      ...
      const response=await axios.post(bacendUrl+'/api/product/add',formData,{headers:{token}})
  ...
  return (
    <form onSubmit={onSubmit} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='w-full'>
        ...
        <textarea onChange={(e)=>setDescription(e.target.value)} value={description} className='w-full mx-w-[500px] px-3 py-2' type="text" placeholder='Write the description' required />
      </div>


    <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
      <div>
        <p className='mb-2'>Product Catregory</p>
      ...
      <div>
        <p className='mb-2'>Sub Catregory</p>
        <select onChange={(e)=>setSubCategory} className='w-full px-3 py-2'>
      ...
      <div>
        ...
        <input onChange={(e)=>setPrice(e.target.value)} value={price} className='w-full px-3 py-2 sm:w-[120px]' type="number"placeholder='20' min={1} />
      </div>
    </div>
